...
class TreeSearchAgent:
    ...
    def action(self, environment):
        ...
        for i in range(self.hyper_parameters.num_simulations):
            ...
            action = self.model.simulated_action(e.state, e.history, possible_actions_list, e.turn)
        ...
        return self.model.real_action(environment.state, environment.history, possible_actions_list, environment.turn)
    ...
    def _find_action_value(self,action, environment):
        ...
        state, history, turn, possible_actions, _, total_rewards, done = environment.move(action)
        while not done:
            ...
            a = self.model.simulated_action(state, history, possible_actions_list, turn)
            state_action_turns.append((state.to_hashable_state(turn), a, turn))
            state, history, turn, possible_actions, _, total_rewards, done = environment.move(a)
